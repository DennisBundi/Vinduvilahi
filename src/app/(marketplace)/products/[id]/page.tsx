...
const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://vinduvilahi.com';
...
export async function generateMetadata({
  ...
  if (!product) {
    return { title: 'Product Not Found - Vindu Vilahi' };
  }
  ...
  return {
    title: `${product.name} - Vindu Vilahi`,
    description: product.description
      ? product.description.slice(0, 160)
      : `Shop ${product.name} at Vindu Vilahi. KSh ${price}. Fast delivery across Kenya.`,
    openGraph: {
      title: `${product.name} - Vindu Vilahi`,
      description: product.description
        ? product.description.slice(0, 160)
        : `Shop ${product.name} at Vindu Vilahi.`,
      type: 'website',
